            ...
            void filterKeyPoints(std::vector<visy::extractors::KeyPoint3D>& keypoints, std::vector<visy::extractors::KeyPoint3D>& keypoints_filtered, float radius) {
                ...
                for (int i = 0; i < keypoints.size(); i++) {
                    if (valid_mask[i]) {
                        ...
                        kdtree.radiusSearchT(searchPoint, radius, found_indices, indices_distances);

                        float search_max = 0.0f;
                        int search_max_index = -1;
                        for (int index = 0; index < found_indices.size(); index++) {
                            valid_mask[found_indices[index]] = false;
                            if (keypoints[found_indices[index]].size > search_max) {
                                search_max = keypoints[found_indices[index]].size;
                                search_max_index = found_indices[index];
                            }
                        }
                        if (search_max_index >= 0) {
                            valid_mask[search_max_index] = true;
                        }
