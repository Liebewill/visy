...
namespace visy
{
  namespace extractors
  {
    ...
    KeyPoint3D::KeyPoint3D (cv::Vec4f line, pcl::PointCloud<PointType>::Ptr cloud) : cv::KeyPoint ()
    {
      ...
      this->direction = cv::Point3f(dx, dy, dz);
      this->pt3D_1 = p03D;
      this->pt3D_2 = p13D;
    }
    ...
    KeyPoint3D::draw3DKeyPoints3D (pcl::visualization::PCLVisualizer& viewer, std::vector<visy::extractors::KeyPoint3D>& keypoints, cv::Scalar color, std::string name)
    {
      ...
      for (int i = 0; i < keypoints.size(); i++)
      {
        ...
        vstart << kp.pt3D.x - kp.direction.x / 2.0f, kp.pt3D.y - kp.direction.y / 2.0f, kp.pt3D.z - kp.direction.z / 2.0f;
        vend << kp.pt3D.x + kp.direction.x / 2.0f, kp.pt3D.y + kp.direction.y / 2.0f, kp.pt3D.z + kp.direction.z / 2.0f;
