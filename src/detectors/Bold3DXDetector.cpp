...
namespace visy
{
  namespace detectors
  {
    ...
    Bold3DXDetector::Bold3DXDetector (std::vector<float>& sizes, int n_bins, bool filter_occlusion, float zone_radius, float zone_slice, float area_normals_angular_th, float area_max_distance)
    {
      ...
      this->descriptor = new visy::descriptors::Bold3DDescriptorMultiBunch(n_bins, sizes, new visy::descriptors::DFunctionB2D(n_bins), visy::descriptors::Bold3DDescriptorMultiBunch::BUNCH_METHOD_KNN);
