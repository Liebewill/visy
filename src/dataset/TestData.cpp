...
namespace visy
{
  namespace dataset
  {
    ...
    std::string TestData::BASE_PATH = "/home/daniele/iros_dataset/";
    std::string TestData::TRAINING_PATH = "training_data/";
    std::string TestData::SETS_PATH = "test_set/";
    std::string TestData::ANNOTATION_PATH = "annotations/";
    std::vector<Model>* TestData::models = new std::vector<Model>();
    std::vector<SetScene>* TestData::scenes = new std::vector<SetScene>();
    ...
    TestData::init ()
    {
      TestData::models->push_back(Model("asus_box", 37));
      TestData::models->push_back(Model("burti", 60));
      TestData::models->push_back(Model("canon_camera_bag", 38));
      TestData::models->push_back(Model("cisco_phone", 19));
      TestData::models->push_back(Model("coffee_container", 16));
      TestData::models->push_back(Model("felix_ketchup", 20));
      TestData::models->push_back(Model("fruchtmolke", 49));
      TestData::models->push_back(Model("jasmine_green_tea", 41));
      TestData::models->push_back(Model("muller_milch_banana", 20));
      TestData::models->push_back(Model("muller_milch_shoko", 19));
      TestData::models->push_back(Model("opencv_book", 12));
      TestData::models->push_back(Model("red_mug_white_spots", 79));
      TestData::models->push_back(Model("skull", 39));
      TestData::models->push_back(Model("strands_mounting_unit", 56));
      TestData::models->push_back(Model("toilet_paper", 16));
      TestData::models->push_back(Model("water_boiler", 19));
      TestData::models->push_back(Model("yellow_toy_car", 19));

      //SCENES
      TestData::scenes->push_back(SetScene(1, 6));
      TestData::scenes->push_back(SetScene(2, 6));
      TestData::scenes->push_back(SetScene(3, 13));
      TestData::scenes->push_back(SetScene(4, 15));
      TestData::scenes->push_back(SetScene(5, 9));
      TestData::scenes->push_back(SetScene(6, 12));
      TestData::scenes->push_back(SetScene(7, 7));
      TestData::scenes->push_back(SetScene(8, 7));
      TestData::scenes->push_back(SetScene(9, 8));
      TestData::scenes->push_back(SetScene(10, 6));
      TestData::scenes->push_back(SetScene(11, 8));
      TestData::scenes->push_back(SetScene(12, 7));
      TestData::scenes->push_back(SetScene(13, 17));
      TestData::scenes->push_back(SetScene(14, 15));
      TestData::scenes->push_back(SetScene(15, 12));
    }

    Model
    TestData::findModelByName (std::string simpleName)
    {
      for (int i = 0; i < TestData::models->size(); i++)
      {
        if (TestData::models->at(i).name == simpleName)
        {
          return TestData::models->at(i);
        }
      }
    ...
    void
    TestData::loadModel (std::string simpleName, int view_number, pcl::PointCloud<PointType>::Ptr full_model, pcl::PointCloud<PointType>::Ptr model, cv::Mat& rgb_model, Eigen::Matrix4f& pose)
    {
    ...
    void
    TestData::loadScene (int set_number, int scene_number, pcl::PointCloud<PointType>::Ptr scene, cv::Mat& rgb_scene)
    {
      std::stringstream path;
    ...
    void
    TestData::loadModel (std::string simpleName, int view_number, pcl::PointCloud<PointType>::Ptr full_model, pcl::PointCloud<PointType>::Ptr model, cv::Mat& rgb_model, cv::Mat& rgb_full_model, Eigen::Matrix4f& pose)
    {
     ...
     */
    void
    TestData::loadIndices (std::string path, std::vector<int>& indices)
    {
    ...
    std::string
    TestData::paddedNumber (int number, int length, bool after)
    {
      std::stringstream id;
      id << number;
      int pad = length;
    ...
    void
    TestData::loadPoseFromFile (std::string path, Eigen::Matrix4f& pose)
    {
      std::ifstream infile(path.c_str());
    ...
    void
    TestData::loadPoseFromSceneFile (std::string model_name, int set_number, int scene_number, int model_index, Eigen::Matrix4f& pose)
    {
      std::stringstream ss;
    ...
    void
    TestData::loadAnnotiationsFromSceneFile (std::string model_name, int set_number, int scene_number, std::vector<Annotation>& annotations)
    {
      annotations.clear();
    ...
    void
    TestData::loadFullAnnotiationsFromSceneFile (int set_number, int scene_number, std::vector<Annotation>& annotations)
    {
      annotations.clear();
      for (int i = 0; i < TestData::models->size(); i++)
      {
        std::vector<Annotation> instances;
        TestData::loadAnnotiationsFromSceneFile((*TestData::models)[i].name, set_number, scene_number, instances);
        annotations.insert(annotations.end(), instances.begin(), instances.end());
      }
    }

    int
    TestData::checkInstancesNumber (std::string model_name, std::vector<Annotation>& annotations)
    {
      int counter = 0;
    ...
    bool
    TestData::checkHV (std::string model_name, Eigen::Matrix4f pose, std::vector<Annotation>& annotations, float distance_th, float rot_th)
    {
      for (int i = 0; i < annotations.size(); i++)
      {
        if (annotations[i].model_name == model_name)
    ...
    bool
    TestData::checkPoses (std::string model_name, std::vector<Eigen::Matrix4f>& poses, std::vector<Annotation>& annotations)
    {
      for (int i = 0; i < annotations.size(); i++)
      {
        if (annotations[i].model_name == model_name)
    ...
    void
    TestData::fetchFullModel (std::string model_name, int views_max_number, std::vector<visy::extractors::KeyPoint3D>& keypoints, cv::Mat& descriptor, pcl::PointCloud<PointType>::Ptr& cloud, Eigen::Matrix4f& reference_pose, visy::detectors::Detector* detector)
    {
    ...
    void
    TestData::fetchFullModelSimple (std::string model_name, int views_max_number, std::vector<visy::extractors::KeyPoint3D>& keypoints, pcl::PointCloud<PointType>::Ptr& cloud, Eigen::Matrix4f& reference_pose, visy::detectors::Detector* detector)
    {
      std::vector<visy::extractors::KeyPoint3D> keypoints_temp;
